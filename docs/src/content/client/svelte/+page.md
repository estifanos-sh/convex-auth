...
`@estifanos-sh/convex-auth/svelte` bridges an app-owned browser auth client into a
...
```svelte
<script lang="ts">
  import { SignedIn, SignedOut, AuthLoading, useConvexAuth } from "@estifanos-sh/convex-auth/svelte";
