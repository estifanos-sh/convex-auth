...
Install the first-party `@robelest/convex-auth` skills to give a coding agent
...
```bash
npx skills add robelest/convex-auth
```
...
| Skill                         | Use it for                                                        |
| ----------------------------- | ----------------------------------------------------------------- |
| `robelest-convex-auth`        | Route a task and distinguish this package from `@convex-dev/auth` |
| `robelest-convex-auth-setup`  | Install, add providers, connect a framework, and verify sign-in   |
| `robelest-convex-auth-review` | Audit security, correctness, protocol behavior, and launch safety |

Skills trigger automatically when an agent recognizes the task. Invoke one
explicitly with `/robelest-convex-auth-setup` in Claude Code, Cursor, and GitHub
Copilot, or `$robelest-convex-auth-setup` in Codex.
...
```text
Add @robelest/convex-auth with GitHub sign-in to this SvelteKit app. Preserve
the existing Convex client, use the current package API, and verify sign-in and
sign-out in the browser.
```

```text
Review this @robelest/convex-auth integration for cross-tenant access,
