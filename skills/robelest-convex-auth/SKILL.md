...
description: Route work involving @robelest/convex-auth to the correct current workflow and source of truth. Use when adding, changing, debugging, reviewing, upgrading, or explaining Robelest Convex Auth; when working with defineAuth, auth providers, auth.config.ts, auth.http(), auth contexts, React or Svelte bindings, passkeys, OAuth, MCP OAuth, SSO, sessions, groups, or permissions; or when a request could be confused with the separate @convex-dev/auth package.
---

# Robelest Convex Auth

Route `@robelest/convex-auth` work to an outcome-focused procedure. Keep the
installed package and project code—not model memory—as the source of truth.

## Route the task

- Use `robelest-convex-auth-setup` to install the package, add or change a
  provider, connect a client framework, or prove a sign-in flow works.
- Use `robelest-convex-auth-review` to audit correctness, security,
...
2. Read the installed `@robelest/convex-auth/package.json`, exports, and `.d.ts`
   files. They describe the version the app actually runs.
3. Use `https://convex-auth.estifanos.com/llms.txt` to locate the current
...
`@robelest/convex-auth` and `@convex-dev/auth` are different packages with
