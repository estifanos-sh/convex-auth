---
name: robelest-convex-auth-setup
description: Install, configure, upgrade, or extend @robelest/convex-auth in an existing Convex application and verify a real sign-in flow. Use for initial setup; adding password, OAuth, passkey/WebAuthn, email, phone, anonymous, TOTP, device, or enterprise connection providers; wiring auth.config.ts and HTTP routes; connecting React, Svelte, Expo, or SSR clients; configuring MCP OAuth; or repairing an incomplete setup. Do not use for @convex-dev/auth.
---

# Set up Robelest Convex Auth
...
Use `https://convex-auth.estifanos.com/llms.txt` as the documentation index.
...
```bash
pnpx @robelest/convex-auth setup --app-url "http://localhost:5173"
```
...
rules. Import providers from `@robelest/convex-auth/providers` or the matching
...
```bash
pnpx @robelest/convex-auth doctor
```
